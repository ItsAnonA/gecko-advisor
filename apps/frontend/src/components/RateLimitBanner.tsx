...
export default function RateLimitBanner({ rateLimit, isPro = false }: RateLimitBannerProps) {
  ...
  if (isPro) {
    return (
      <div className="bg-advisor-600/20 border border-advisor-500/30 rounded-lg px-4 py-3">
        <div className="flex items-center gap-2 text-sm">
          <svg className="w-5 h-5 text-advisor-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
          </svg>
          <span className="font-semibold text-advisor-300 drop-shadow-[0_1px_3px_rgba(0,0,0,0.8)]">Pro: Unlimited Scans</span>
        </div>
      </div>
    );
  }
  ...
  if (hasScansRemaining) {
    return (
      <div className="bg-trust-600/20 border border-trust-600/30 rounded-lg px-4 py-3">
        <div className="flex items-center gap-2 text-sm">
          <svg className="w-5 h-5 text-trust-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
          </svg>
          <span className="text-light-secondary drop-shadow-[0_1px_3px_rgba(0,0,0,0.8)]">
            <span className="font-semibold text-trust-300">{scansRemaining}</span> scan{scansRemaining !== 1 ? 's' : ''} remaining today
          </span>
        </div>
        <p className="text-xs text-light-tertiary mt-1 drop-shadow-[0_1px_3px_rgba(0,0,0,0.8)]">
  ...
  // Free user at limit
  return (
    <div className="bg-score-caution/20 border border-score-caution/30 rounded-lg p-4">
      <div className="flex items-start gap-3">
        <svg className="w-5 h-5 text-score-caution flex-shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
        </svg>
        <div className="flex-1">
          <h3 className="text-sm font-semibold text-light-primary drop-shadow-[0_1px_3px_rgba(0,0,0,0.8)]">Daily Limit Reached</h3>
          <p className="text-sm text-light-secondary mt-1 drop-shadow-[0_1px_3px_rgba(0,0,0,0.8)]">
            You've used all your scans today. Your limit resets at {resetTime}.
          </p>
          <p className="text-xs text-light-tertiary mt-2 drop-shadow-[0_1px_3px_rgba(0,0,0,0.8)]">
