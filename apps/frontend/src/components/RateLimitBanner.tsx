...
export default function RateLimitBanner({ rateLimit, isPro = false }: RateLimitBannerProps) {
  ...
  if (hasScansRemaining) {
    return (
      <div className="bg-trust-100 border border-trust-200 rounded-lg px-4 py-3">
        <div className="flex items-center gap-2 text-sm">
          <svg className="w-5 h-5 text-trust-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
          </svg>
          <span className="text-zinc-600">
            <span className="font-semibold text-trust-700">{scansRemaining}</span> scan{scansRemaining !== 1 ? 's' : ''} remaining today
          </span>
        </div>
    ...
    <div className="bg-amber-50 border border-amber-200 rounded-lg p-4">
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1">
          <h3 className="text-sm font-semibold text-zinc-900">Daily Limit Reached</h3>
