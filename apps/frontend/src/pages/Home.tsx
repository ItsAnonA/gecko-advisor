  ...
  return (
    <>
      ...
      <header className="text-center space-y-4 py-4 md:py-8">
        {/* Trust Signals - Above headline with unique pill sizes */}
        <div className="flex flex-wrap items-center justify-center gap-2 md:gap-3 mb-4">
          {/* Small pill - Open Source */}
          <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-advisor-50 border border-advisor-200 text-xs font-medium text-advisor-700">
            <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M10 20l4-16m4 4l4 4-4 4M6 16l-4-4 4-4" />
            </svg>
            Open Source
          </span>
          {/* Medium pill - No Account */}
          ...
          <a
            ...
            className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-zinc-100 border border-zinc-300 text-xs font-medium text-zinc-700 hover:bg-zinc-200 hover:border-zinc-400 transition-colors"
          >
          ...
          {/* Large pill - Scans count (most prominent) */}
          {stats && stats.totalScans > 0 && (
            <Link
              to="/reports"
              className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-gradient-to-r from-advisor-500 to-emerald-500 border-2 border-advisor-400 text-sm font-bold text-white shadow-md hover:shadow-lg hover:scale-105 transition-all duration-200 cursor-pointer"
